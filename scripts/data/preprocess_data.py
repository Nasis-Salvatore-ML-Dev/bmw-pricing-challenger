...
def split_raw_data(df: pd.DataFrame, train_size: float = 0.6, val_size: float = 0.2,
    ...
    val_df, test_df = train_test_split(temp_df, train_size=val_ratio, random_state=random_state, shuffle=True)
...
def extract_bmw_series(model_key: str) -> str:
    ...
    if 'X1' in model_key:
        ...
        return 'X7'
    elif '1' in model_key and any(x in model_key for x in ['114', '116', '118', '120', '125', '128', '130', '135', '140']):
        return '1_series'
    elif '2' in model_key and any(x in model_key for x in ['214', '216', '218', '220', '225', '228', '230', '235', '240']):
        return '2_series'
    elif '3' in model_key and any(x in model_key for x in ['316', '318', '320', '325', '328', '330', '335', '340']):
        return '3_series'
    elif '4' in model_key and any(x in model_key for x in ['418', '420', '425', '428', '430', '435', '440']):
        return '4_series'
    elif '5' in model_key and any(x in model_key for x in ['518', '520', '523', '525', '528', '530', '535', '540', '550']):
        return '5_series'
    elif '6' in model_key and any(x in model_key for x in ['620', '630', '640', '650']):
        return '6_series'
    elif '7' in model_key and any(x in model_key for x in ['730', '735', '740', '745', '750', '760']):
        return '7_series'
    elif '8' in model_key:
        return '8_series'
...
def create_temporal_features(df: pd.DataFrame, sold_date_col: str = 'sold_at') -> pd.DataFrame:
    ...
    if 'car_age_years' not in df.columns:
        logger.info("  ℹ️  car_age_years not found, calculating from registration_date and sold_at...")
...
def main():
    ...
    with open(metadata_path, 'w') as f:
        ...
        f.write(f"CRITICAL: All features are observable at prediction time\n")
        f.write(f"NO price-dependent features (no leakage)\n")
