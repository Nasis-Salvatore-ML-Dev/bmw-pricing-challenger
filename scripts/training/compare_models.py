...
from src.evaluation.metrics import calculate_all_metrics  # noqa: E402
...
from src.models.xgboost_trainer import load_best_xgboost_params, train_xgboost_model  # noqa: E402
...
def compare_models():
    ...
    rf_metrics, rf_time = train_and_evaluate_random_forest(X_train, y_train, X_val, y_val, encoders)

    xgb_metrics, xgb_time = train_and_evaluate_xgboost(X_train, y_train, X_val, y_val, encoders)
...
def generate_comparison_report(rf_metrics, rf_time, xgb_metrics, xgb_time):
    ...
    tr_improvement = (
        (rf_metrics["tail_rate"] - xgb_metrics["tail_rate"]) / rf_metrics["tail_rate"]
    ) * 100
...
def determine_winner(rf_metrics, xgb_metrics):
    ...
    if xgb_score > rf_score:
        logger.info("\n🏆 WINNER: XGBoost")
        logger.info("   XGBoost is the better model for BMW pricing")
        logger.info(
            f"   MAE improvement: {((rf_metrics['mae'] - xgb_metrics['mae']) / rf_metrics['mae']) * 100:.1f}%"
        )
    elif rf_score > xgb_score:
        logger.info("\n🏆 WINNER: Random Forest")
        logger.info("   Random Forest is the better model for BMW pricing")
    else:
        logger.info("\n🤝 TIE: Both models perform equally")
        logger.info("   Consider ensemble or model selection based on deployment constraints")
    ...
    comparison_data = {
        ...
        "improvements": {
            "mae_improvement_pct": float(
                ((rf_metrics["mae"] - xgb_metrics["mae"]) / rf_metrics["mae"]) * 100
            ),
            "r2_improvement_pct": float(
                ((xgb_metrics["r2"] - rf_metrics["r2"]) / rf_metrics["r2"]) * 100
            ),
            "tail_rate_improvement_pct": float(
                ((rf_metrics["tail_rate"] - xgb_metrics["tail_rate"]) / rf_metrics["tail_rate"])
                * 100
            ),
        },
