...
def perform_residual_analysis(
    ...
    logger.info("=" * 80)
    logger.info(f"  Tail samples: {len(tail_samples):,} ({len(tail_samples)/len(analysis_df)*100:.1f}%)")
    logger.info(f"  Normal samples: {len(normal_samples):,} ({len(normal_samples)/len(analysis_df)*100:.1f}%)")
...
def analyze_by_price_segment(df: pd.DataFrame):
    ...
    logger.info(f"   This segment needs feature engineering improvements")
...
def create_residual_plots(df: pd.DataFrame, model_name: str):
    ...
    axes[1, 1].plot([df['actual_euro'].min(), df['actual_euro'].max()],
...
def save_tail_analysis(tail_df: pd.DataFrame, model_name: str):
    ...
    logger.info(f"   Contains top 100 worst predictions for investigation")
